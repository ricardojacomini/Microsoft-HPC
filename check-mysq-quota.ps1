...
function Get-PriceForVmSize {
    ...
    if ($PriceCache.ContainsKey($VmSize)) {
        return $PriceCache[$VmSize]
    }

    # Build filter: productName contains 'Virtual Machines', armRegionName equals region, skuName equals VmSize
    $filter = "serviceFamily eq 'Compute' and armRegionName eq '$Region' and skuName eq '$VmSize' and priceType eq 'Consumption'"
    ...
    try {
        ...
        if ($response.Items -and $response.Items.Count -gt 0) {
            $item = $response.Items | Where-Object { $_.unitOfMeasure -eq '1 Hour' } | Select-Object -First 1
            if (-not $item) { $item = $response.Items[0] }
            $price = [decimal]$item.unitPrice
            # Cache and return
            $PriceCache[$VmSize] = $price
            return $price
        }
    } catch {
        # Quiet failure; cache 'null' to avoid repeated failing calls
        $PriceCache[$VmSize] = $null
        return $null
    }
...
foreach ($region in $regions) {
    ...
    # Determine which quota name to look for (derived from VM size)
        ...
        if ($quota) {
            ...
            # Attempt to fetch price (first region used) and format
            $price = Get-PriceForVmSize -VmSize $vm -Region $region
            if ($price) { $priceText = ("${0:0.0000}" -f $price) } else { $priceText = 'N/A' }
            if ($maxVMs -gt 0) {
                Write-Host ("  {0,-28} Available vCPUs: {1,5} -> Max VMs: {2,6}  Price/hr (USD): {3}" -f $vm, $available, $maxVMs, $priceText) -ForegroundColor Green
            } else {
                Write-Host ("  {0,-28} Available vCPUs: {1,5} -> Max VMs: {2,6}  Price/hr (USD): {3}" -f $vm, $available, $maxVMs, $priceText) -ForegroundColor Red
            }
        } else {
