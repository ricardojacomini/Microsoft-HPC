...
@description('Optional AAD object id (user/service principal/managed identity) to receive Key Vault Certificates Officer role')
param certOfficerPrincipalId string = ''

var vaultPrefix = '${_clusterName}-kv'

module keyVault 'shared/key-vault-with-cert-ssa.bicep' = {
  name: 'KeyVaultWithCert'
  params: {
    vaultNamePrefix: vaultPrefix
    location: resourceGroup().location
    certOfficerPrincipalId: certOfficerPrincipalId
  }
}
