...
param domainControllerVMSize string = 'Standard_D4s_v5'

@description('The data disk size in GB for the domain controller.')
@minValue(32)
@maxValue(1023)
param domainControllerDataDiskSize int = 256

@description('The data disk type for the domain controller.')
param domainControllerDataDiskType DiskType = 'Standard_HDD'
...
param headNodeOsDiskType DiskType = 'Standard_HDD'
...
param headNodeVMSize string = 'Standard_D4s_v5'
...
param computeNodeVMSize string = 'Standard_D4s_v5'
...
@description('Specify whether to install InfiniBand driver on the head node. This overrides the autoInstallInfiniBandDriver setting specifically for the head node.')
param installIBDriverOnHeadNode YesOrNo = 'No'

@description('Specify whether to install InfiniBand driver on the compute nodes. This overrides the autoInstallInfiniBandDriver setting specifically for the compute nodes.')
param installIBDriverOnComputeNodes YesOrNo = 'Yes'
...
var installIBDriverOnHN = (installIBDriverOnHeadNode == 'Yes')
var installIBDriverOnCN = (installIBDriverOnComputeNodes == 'Yes')
...
module vnet 'shared/vnet-with-dc.bicep' = {
  ...
  params: {
    ...
    vNetName: vNetName
    dcDataDiskSize: domainControllerDataDiskSize
    dcDataDiskType: domainControllerDataDiskType == 'Standard_HDD' ? 'Standard_LRS' : (domainControllerDataDiskType == 'Standard_SSD' ? 'StandardSSD_LRS' : 'Premium_LRS')
  }
}
...
module headNode 'shared/head-node.bicep' = {
  ...
  params: {
    ...
    installIBDriver: hnRDMACapable && (autoEnableInfiniBand || installIBDriverOnHN)
...
module computeNodes 'shared/compute-node.bicep' = [
  for i in range(0, computeNodeNumber): if (!useVmssForCN) {
    ...
    params: {
      ...
      installRDMADriver: (cnRDMACapable && (autoEnableInfiniBand || installIBDriverOnCN))
      enableAcceleratedNetworking: (enableAcceleratedNetworking == 'Yes')
      certSettings: certSettings
...
// Custom Windows InfiniBand driver extension for head node
resource headNodeIBDriver 'Microsoft.Compute/virtualMachines/extensions@2019-03-01' = if (hnRDMACapable && installIBDriverOnHN) {
  name: '${_clusterName}/installInfiniBandDriverWindows'
  location: resourceGroup().location
  properties: {
...
// Custom Windows InfiniBand driver extension for compute nodes
resource computeNodeIBDriver 'Microsoft.Compute/virtualMachines/extensions@2019-03-01' = [
  for i in range(0, computeNodeNumber): if (!useVmssForCN && cnRDMACapable && installIBDriverOnCN) {
    name: '${_computeNodeNamePrefix}${padLeft(string(i), 3, '0')}/installInfiniBandDriverWindows'
    location: resourceGroup().location
    properties: {
      publisher: 'Microsoft.HpcCompute'
      type: 'InfiniBandDriverWindows'
