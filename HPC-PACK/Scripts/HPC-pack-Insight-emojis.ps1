...
function Repair-NetworkConnectivity {
    ...
    try { netsh winsock reset | Out-Null; Write-Host "  OK Winsock reset" -ForegroundColor Green } catch { Write-Host "  ERROR Winsock reset failed: $($_.Exception.Message)" -ForegroundColor Red }
    try { ipconfig /flushdns | Out-Null; Write-Host "  OK Flushed DNS" -ForegroundColor Green } catch { Write-Host "  ERROR Flush DNS failed: $($_.Exception.Message)" -ForegroundColor Red }
    foreach($p in $Ports){
        try{
            $rn = "HPC-Port-$p"
            if(-not (Get-NetFirewallRule -DisplayName $rn -ErrorAction SilentlyContinue)){
                ...
                Write-Host "  OK Firewall rule for $p" -ForegroundColor Green
            } else { Write-Host "  INFO Firewall rule exists for $p" -ForegroundColor Cyan }
        } catch { Write-Host "  ERROR Firewall rule $p failed: $($_.Exception.Message)" -ForegroundColor Red }
    }
}
    ...
    try {
        ...
        foreach ($port in $Ports) {
            try {
                ...
                if ($isOpen) {
                    Write-Host ("OK Port {0} is open on {1}" -f $port, $NodeName) -ForegroundColor Green
                } elseif (-not $Quiet) {
                    Write-Warning ("ERROR Port {0} is closed or unreachable on {1}" -f $port, $NodeName)
                }
            } catch {
                if (-not $Quiet) { Write-Warning ("WARNING Error testing port {0} on {1}: {2}" -f $port, $NodeName, $_.Exception.Message) }
            }
        }
    } catch {
    if (-not $Quiet) { Write-Warning ("WARNING Test-HpcNodePorts failed: {0}" -f $_.Exception.Message) }
    }
}
...
function Invoke-NetworkFix {
    ...
    try {
        ...
        if ($gw) {
            ...
            if ($ok) { Write-Host "  OK Default gateway reachable ($gw)" -ForegroundColor Green }
            else { Write-Host "  ERROR Default gateway not responding to ping ($gw)" -ForegroundColor Red }
        } else {
            Write-Host "  WARNING No default route found (IPv4 or IPv6)" -ForegroundColor Yellow
        }
        # DNS reachability quick check for common hosts
        $dnsHosts = @('microsoft.com','azure.com')
        foreach ($h in $dnsHosts) {
            try {
                if (Resolve-DnsName $h -ErrorAction Stop -Verbose:$false) { Write-Host "   OK $h" -ForegroundColor Green }
            } catch { Write-Host "   ERROR $h" -ForegroundColor Red }
        }
    }
    catch { }
...
function Invoke-CommandTest {
    ...
    try {
        ...
        if($ov){
            Write-Host "  OK Cluster: $($ov.ClusterName) | Version: $($ov.Version) | Nodes: $($ov.TotalNodeCount)" -ForegroundColor Green
        }
    } catch { Write-Host "  ERROR Get-HpcClusterOverview: $($_.Exception.Message)" -ForegroundColor Red }
}
...
function Invoke-NodeValidation {
    ...
    if (-not (Import-HpcModule -Quiet)) { Write-Host "  ERROR HPC module not available. Skipping." -ForegroundColor Red; return }
    ...
    Write-Host "  NODES: $($nodes.Count) | Online: $online | Healthy: $healthy" -ForegroundColor White
        $crit = $nodes | Where-Object { $_.NodeState -ne 'Online' -or $_.HealthState -ne 'OK' } | Select-Object -First 10
        if ($crit) {
            Write-Host "  WARNING Nodes needing attention:" -ForegroundColor Yellow
    ...
    Write-Host "  WARNING No node data returned" -ForegroundColor Yellow
    }
...
function Invoke-NodeConfig {
    ...
    } else { Write-Host "  WARNING: HPC registry key not found" -ForegroundColor Yellow }
